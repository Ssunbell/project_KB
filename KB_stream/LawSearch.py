...
class LawSearcher():
    def __init__(self):
        with open('./data/law.json', 'r', encoding='utf-8') as f:
            self.law = json.load(f)

        with open('./data/law_vectors.json', 'r', encoding='utf-8') as f:
            self.law_vectors = json.load(f)
        
        self.titles = list(self.law.keys())

        self.model = Word2Vec.load('./data/law_w2v')
    ...
    def show_result(self, result): # streamlit
        if result is None:
            ...
            try:
                ...
                st.warning("관련 조문이 존재하지 않습니다.")
            jangs = self.law[result[0]].keys()
            add_selectbox02 = st.selectbox('장 선택', jangs)
            jos = self.law[result[0]][add_selectbox02].keys()
            add_selectbox03 = st.selectbox('조 선택', jos)
            contents = self.law[result[0]][add_selectbox02][add_selectbox03].replace('{', '').replace('}', '')
            st.text_area('내용', contents, height=500)
